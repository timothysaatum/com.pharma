...
fn temp_db_path(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "crsqlite_spike_{label}_{}_{}.db",
        std::process::id(),
        id,
    ))
}

fn open_db(path: &Path, load_crsqlite: bool) -> Connection {
...
fn connect_existing(path: &Path, load_crsqlite: bool) -> Connection {
...
fn step3_latency_measurement() {
    ...
    let conn = open_db(&plain_path, false);
    create_schema(&conn);
    ...
    // ── CRR table ──
    let crr_path = temp_db_path("crr");
    let crr = open_db(&crr_path, true);
    create_schema(&crr);
    crr.execute_batch("SELECT crsql_as_crr('branch_inventory')")
        .expect("crr");
    ...
    measure_latency("SELECT from CRR table (by id)", 100, || {
        let _: i64 = crr
            .query_row(
                "SELECT quantity FROM branch_inventory WHERE id = ?1",
                params![id2],
                |r| r.get(0),
            )
            .unwrap_or(0);
    ...
    measure_latency("SELECT from CRR table (full scan)", 100, || {
        let mut stmt = crr
            .prepare("SELECT COUNT(*) FROM branch_inventory")
            .unwrap();
...
fn step4_wal_savepoint_crsqlite() {
    ...
    let db_path = temp_db_path("step4");
    ...
    create_schema(&conn);
    conn.execute_batch("SELECT crsql_as_crr('branch_inventory')")
        .expect("crr");
    ...
    assert!(
        change_count > 0,
        "INSERT must produce crsql_changes entries"
    );

    // 4c. Savepoint with commit — changes should be tracked
    conn.execute_batch("SAVEPOINT sp_commit")
        .expect("savepoint");
    conn.execute(
    ...
    println!(
        "  4c. crsql_changes quantity entries after committed savepoint: {changes_after_commit}"
    );
    assert!(
        changes_after_commit >= 1,
        "Committed savepoint must produce change entries"
    );
    ...
    conn.execute_batch("SAVEPOINT sp_rollback")
        .expect("savepoint");
    conn.execute(
    ...
    conn.execute_batch("ROLLBACK TO sp_rollback")
        .expect("rollback");
    conn.execute_batch("RELEASE sp_rollback")
        .expect("release after rollback");

    let qty_after_rollback: i64 = conn
        .query_row(
            "SELECT quantity FROM branch_inventory WHERE id = ?1",
            params![row_id],
            |r| r.get(0),
    ...
    assert_eq!(
        leaked, 0,
        "Rolled-back changes must not appear in crsql_changes"
    );
    ...
    let loc: String = conn
        .query_row(
            "SELECT location FROM branch_inventory WHERE id = ?1",
            params![row_id],
            |r| r.get(0),
        )
        .unwrap();
    let reserved: i64 = conn
        .query_row(
            "SELECT reserved_quantity FROM branch_inventory WHERE id = ?1",
            params![row_id],
            |r| r.get(0),
        )
        .unwrap();
    ...
    println!(
        "  4f. crsql_finalize function exists: {}",
        finalize_check.is_ok()
    );
...
fn step5_concurrency() {
    ...
    create_schema(&main);
    main.execute_batch("SELECT crsql_as_crr('branch_inventory')")
        .expect("crr");
        ...
        handles.push(thread::spawn(move || {
            for _ in 0..iterations {
                ...
                let conn = match Connection::open(&path) {
                    Ok(c) => c,
                    Err(e) => {
                        errs.lock().unwrap().push(format!("reader {ri} open: {e}"));
                        return;
                    }
                };
    ...
    let final_qty: i64 = verify
        ...
        .unwrap();
    println!(
        "  Final quantity: {final_qty} (expected 100+iterations-1 = {})",
        100 + iterations - 1
    );

    assert!(
        error_list.is_empty(),
        "Concurrent reads must not produce errors"
    );
    assert_eq!(
        reader_results.len(),
        reader_count,
        "All readers must complete"
    );
