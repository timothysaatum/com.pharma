...
class TestSyncSaleItems:
    ...
    async def test_push_sale_rejects_rx_drug_without_valid_prescription(
        ...
        """Rx-only sales whose prescription is missing everywhere dead-letter.

        The prescription UUID exists in neither Postgres nor the CRR shadow DB
        (never pushed, or its own CRR push was rejected and the shadow row
        tombstoned). Retry is pointless — the prescription can never arrive —
        so the sale must be classified PERMANENTLY_REJECTED so the client
        stops looping and surfaces the sale in the failures panel for the
        user to void or recreate. See sync_service.py `_push_sale` shadow
        probe branch.
        """
        org, branch, user, drugs, customer = setup_test_data
        ...
        assert await db.get(Sale, sale_id) is None
        # Contract check: the failure must carry PERMANENTLY_REJECTED so the
        # client's failure loop hard-sets attempts to MAX_PUSH_ATTEMPTS and
